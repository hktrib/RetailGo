...
func (cc *CategoryCreate) SetPhoto(b []byte) *CategoryCreate {
	cc.mutation.SetPhoto(b)
	return cc
}
...
func (cc *CategoryCreate) SetStore(s *Store) *CategoryCreate {
...
func (cc *CategoryCreate) check() error {
	...
	if _, ok := cc.mutation.Photo(); !ok {
		return &ValidationError{Name: "photo", err: errors.New(`ent: missing required field "Category.photo"`)}
	}
	...
	if _, ok := cc.mutation.StoreID(); !ok {
		return &ValidationError{Name: "store", err: errors.New(`ent: missing required edge "Category.store"`)}
	}
	return nil
}
