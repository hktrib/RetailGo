...
func (cq *CategoryQuery) QueryStore() *StoreQuery {
...
func (cq *CategoryQuery) QueryCategoryItem() *CategoryItemQuery {
	query := (&CategoryItemClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CategoryQuery) Clone() *CategoryQuery {
	...
	return &CategoryQuery{
		...
		withCategoryItem: cq.withCategoryItem.Clone(),
...
func (cq *CategoryQuery) WithStore(opts ...func(*StoreQuery)) *CategoryQuery {
	query := (&StoreClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CategoryQuery) WithCategoryItem(opts ...func(*CategoryItemQuery)) *CategoryQuery {
	query := (&CategoryItemClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withCategoryItem = query
	return cq
}
...
func (cq *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := cq.withStore; query != nil {
		if err := cq.loadStore(ctx, query, nodes, nil,
			func(n *Category, e *Store) { n.Edges.Store = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withCategoryItem; query != nil {
		if err := cq.loadCategoryItem(ctx, query, nodes,
			func(n *Category) { n.Edges.CategoryItem = []*CategoryItem{} },
			func(n *Category, e *CategoryItem) { n.Edges.CategoryItem = append(n.Edges.CategoryItem, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CategoryQuery) loadItems(ctx context.Context, query *ItemQuery, nodes []*Category, init func(*Category), assign func(*Category, *Item)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(item.FieldID), joinT.C(category.ItemsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(category.ItemsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(category.ItemsPrimaryKey[0]))
...
func (cq *CategoryQuery) loadStore(ctx context.Context, query *StoreQuery, nodes []*Category, init func(*Category), assign func(*Category, *Store)) error {
...
func (cq *CategoryQuery) loadCategoryItem(ctx context.Context, query *CategoryItemQuery, nodes []*Category, init func(*Category), assign func(*Category, *CategoryItem)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(categoryitem.FieldCategoryID)
	}
	query.Where(predicate.CategoryItem(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.CategoryItemColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (cq *CategoryQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withStore != nil {
			_spec.Node.AddColumnOnce(category.FieldStoreID)
		}
	}
