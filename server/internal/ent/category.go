...
func (e CategoryEdges) StoreOrErr() (*Store, error) {
	if e.loadedTypes[1] {
		if e.Store == nil {
...
// was not loaded in eager-loading.
func (e CategoryEdges) CategoryItemOrErr() ([]*CategoryItem, error) {
	if e.loadedTypes[2] {
...
func (*Category) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case category.FieldPhoto:
			values[i] = new([]byte)
		case category.FieldID, category.FieldStoreID:
...
func (c *Category) QueryItems() *ItemQuery {
	return NewCategoryClient(c.config).QueryItems(c)
}

// QueryStore queries the "store" edge of the Category entity.
func (c *Category) QueryStore() *StoreQuery {
	return NewCategoryClient(c.config).QueryStore(c)
}

// QueryCategoryItem queries the "category_item" edge of the Category entity.
func (c *Category) QueryCategoryItem() *CategoryItemQuery {
	return NewCategoryClient(c.config).QueryCategoryItem(c)
}
...
func (c *Category) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", c.Photo))
	builder.WriteString(", ")
